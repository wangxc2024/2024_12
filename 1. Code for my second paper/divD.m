function [N]=divD(x,param,Inoise)
...
signoise=0.005;
tnoise=0.002;
sige2=eye(90)*signoise^2*Inoise;
sigi2=eye(90)*signoise^2*Inoise;

De=game^2*0.5*(1-x).^2.*(dphi.^2.*sige2);
Di=gami^2*0.5*(dphic.^2.*sigi2);
D=[De(1:60,:);Di(61:90,:)];
Ne=-(-game^2*(1-x).*(dphi.^2.*sige2)+game^2*(1-x).^2.*(dphi.*ddphi.*sige2).*[Js;Js;Js]);
N=zeros(90,1);
for i=1:60
    N(i)=Ne(i,i);
end
