...
D=1;
Gn = @(x,param)  diffu(x,param,1,0.1);
t=1;
...
pulse=zeros(30,3);
pulse(1,1)=0;
[xm1,xm2,xr,nstates,weight]=getstatedwm(Jm,G,pulse);
if nstates<3
...
%                 end
    ...
    S_final(ig)=S_modified;
    X_result{ig}(:)=X(:); %将一次退火的结果记录下来
%     kk=kk+1;   
ig
end
...
% 保存工作数据到子文件夹中
save(fullfile(subFolderPath, 'action_m2r.mat'));
