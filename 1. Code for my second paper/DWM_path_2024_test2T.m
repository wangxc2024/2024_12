...
param=paramJm(0.26,0.001);   
pulse1=zeros(30,3); 
y1=getatracedwmnoise(5,xfix1,param,pulse1,0,0);
ya=getatracedwmnoise(5,xfix0,param,pulse1,0,0);
resting=y1(:,5/param.ut);
% resting=C{1,1,1}(:,3);
memory_a=ya(:,5/param.ut);
% memory_a=C{1,1,1}(:,1);
memory_b=C{1,1,1}(:,2);
...
gtype=zeros(1,n_anneal);
% dS=zeros(n_anneal);
% dxk=zeros(n_anneal);
% Tk=zeros(n_anneal);

Xmove=zeros(90,kkk,n_anneal/nn);
% Smove=zeros(n_anneal/nn);
...
%             S_modified = S_modified + real(Som_i(x0,x1,F1, G1 ,param,D,E0,Veff1,avedl1)); %计算全序列的Som%%%%%%%%%%%%%%%%%%
            Somi0(i)=(Som_i(X(:,i),X(:,i+1),F90(t,X(:,i),param), F90(t,X(:,i+1),param),Gn((X(:,i+1)+X(:,i))/2,param),param,D,h));
        end
...
%             %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
            ...
            gv(i)=max(sqrt(dSdx90(:,i)'*dSdx90(:,i)),1e-10);
            for j=1:90
            % if dSdx90(j,i)<0
            %     X1(j,i+1) = X2(j,1)+0.1*(abs(dSdx90(j,i)))/(sum(abs(dSdx90(:,i))))*T_anneal*10;
            % elseif dSdx90(j,i)==0
            %     X1(j,i+1) = X2(j,1);
            % elseif dSdx90(j,i)>0
            %     X1(j,i+1) = X2(j,1)-0.1*(abs(dSdx90(j,i)))/(sum(abs(dSdx90(:,i))))*T_anneal*10;
            ...
            % X1(j,i+1) = X2(j,1)-0.01*((dSdx90(j,i)))/gv(i)*T_anneal*1;
            X1(j,i+1) = X2(j,1)-0.005*((dSdx90(j,i)))/gv(i)*ts*1;
            if X1(j,i+1)<=0
                X1(j,i+1)=-X1(j,i+1);
            end
            end
...
%             S_modified = S_modified + real(Som_i(x0,x1,F1, G1 ,param,D,E0,Veff1,avedl1)); %计算全序列的Som%%%%%%%%%%%%%%%%%%
            Somm(i)=Som_i(X1(:,i),X1(:,i+1),F90(t,X1(:,i),param),F90(t,X1(:,i+1),param),Gn((X1(:,i+1)+X1(:,i))/2,param),param,D,h);
        end                        
        S_modified=real(sum(Somm));
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
        
        dS(kkkk+1)=S_modified - S_initial;
        dxk(kkkk+1)=sum(sqrt(sum((X1-X).^2)))/kkk;
        Tk(kkkk+1)=T_anneal;
        ...
        elseif S_modified > S_initial
            % disp('--------------------------AAA');
            if exp(-7.5*(S_modified-S_initial)/(T_anneal)) > (rand()) %退火过程
                disp('--------------------------BBB');
                gtype(kkkk+1)=2;
                X = (X1-X)*0.1+X; %Y = (Y1-Y)*0.25+Y;
            else
                disp('--------------------------CCC');
                gtype(kkkk+1)=1;
                ...
                for r = Xsample
                    % X(:,r) = 0.5*(X(:,r)+X1(:,r));
                    X(:,r) = 1*X(:,r)+abs(randn())/4*(X1(:,r)-X(:,r));
                end
%                 for r = Ysample
%                     Y(r) = 0.5*(Y(r)+Y1(r));
%                 end
        ...
        T_anneal = 0.5*(1.05-tanh(0.025/nn*(kkkk-185*nn)));  %降低退火温度
        kkkk=kkkk+1;
        mk=floor(kkkk/nn);
        if mk>mmk
            mk
...
% save(fullfile(subFolderPath, 'path_m2r_best_h_direct_241010.mat'));    %24.1010
% save(fullfile(subFolderPath, 'path_r2m_best_h_direct_241011.mat'));    %24.1010
save(fullfile(subFolderPath, 'path_r2m_best_h_direct_241014.mat'));    %24.1010
