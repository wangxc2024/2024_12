...
% x0=X(:,i);
%             x1=X(:,i+1);
%             F0=F90(t,(x1+x0)/2,param);
%             F1=F90(t,(x1+x0)/2,param);
%             G1=Gn((x1+x0)/2,param);
%             divD1 =divD(t,(x1+x0)/2,param);
%             dF1 =dF(t,(x1+x0)/2,param);
%             Veff1 = Veff(x0,x1, F1,dF1, G1,param,D,divD1,avedl);
%              S_initial = Som_i(x0,x1, F1, G1,param,D,E0,Veff1,avedl)
%              ( 2*(E0-Veff1) * ((x1-x0)'*(G1)^-1*(x1-x0)) ).^0.5
%              (F0+F1)'/2 * (G1)^-1 * (x1-x0)
% G1=Gn((x1+x0)/2,param);
% Veff1 =Veff(x0,x1, F1,dF1, G1,param,1,zeros(90,1),avedl)
t=1;
...
Veff = @(x,x1,Fx,dF,Gn,param,D,divD,avedl) ...                          %取中值代入
       - 0.5 * ( trace(dF(1:60,1:60)) - (Fx)' * (Gn)^-1 * divD ) ... 
       - 0.5 .* (Fx)' * (Gn)^-1 * (Fx) ;
...
Som_i = @(x,x1,Fx,Gn,param,D,E,V_eff,avedl) ...                         %取中值代入
        ( 2*(E-V_eff) * ((x1-x)'*(Gn)^-1*(x1-x)) ).^0.5 ...
        - (Fx)' * (Gn)^-1 * (x1-x) ...
        + 100000 * ( (x1-x)'*(x1-x) - avedl^2 ) ;

dSom = @(x_1,x,x1,Fx,Fx1,dFx,Gn,param,divD) ...
        -dFx * (Gn^-1) * (x1-x_1)/2 ...
        +((Fx .* divD)'/(Gn.^2))' .* (x1-x)/2 ...
        +((Fx1 .* divD)'/(Gn.^2))' .* (x-x_1)/2 ;
...
%     T_anneal=0.00001;
    ...
    S_initial0=0;
    sumy=ones(1,90);
    avedl=mean((sumy*gradient(X).^2).^0.5);
    Somi0=zeros(1,kkk-1);
...
%             F0=F90(t,x0,param);
            ...
            dF1 =dF(t,(x1+x0)/2,param);
            Veff1 =Veff(x0,x1,F1,dF1, G1,param,D,divD1,avedl);
%             S_initial0 = S_initial0 + real(Som_i(x0,x1,F1, G1 ,param,D,E0,Veff1,avedl));%%%
            Somi0(i)=(Som_i(x0,x1,F1, G1 ,param,D,E0,Veff1,avedl));
    end
    S_initial0 = real(sum(Somi0));%%%
...
%             F0=F90(t,x0,param);
            ...
            dF1 =dF(t,(x1+x0)/2,param);
            Veff1 =Veff(x0,x1,F1,dF1,G1,param,D,divD1,avedl);
            Somi(i)=(Som_i(x0,x1,F1, G1 ,param,D,E0,Veff1,avedl));
%             S_initial = S_initial + real(Som_i(x0,x1,F1, G1 ,param,D,E0,Veff1,avedl));%%%
        end
        S_initial = real(sum(Somi));%%%
    ...
    %             h=Xt(1,i+1)-Xt(1,i);
                ...
                F1=F90(t,(x1+x0)/2,param);
                F0=F90(t,(x_1+x0)/2,param);
                divD0 =divD(t,x0,param);
                dF1=dF(t,x0,param);
                G1=Gn(x0,param);
                dSdx90(:,i) = dSom(x_1,x0,x1,F0,F1,dF1,G1,param,divD0); %计算的是X(i+1)位置的导数%%%%%%%%%%%%%%%%%
            ...
            parfor i = 1:(kkk-2) %根据S的导数确定每一点的优化
                X2=X(:,i+1);
                ldS=sqrt(dSdx90(:,i)'*dSdx90(:,i));
                for j=1:90                                  % 12.31,WXC: we need to calculate the gradient of Som then find the ward of attemp steps. 

                    X1(j,i+1) = X2(j,1)-0.1*(dSdx90(j,i))/(ldS)*T_anneal^1+0.01/sqrt(90)*T_anneal*randn(1);
            ...
            for j=1:90                                  
                ...
                X1(j,i+1) = X2(j,1)+0.03/sqrt(90)*T_anneal*randn(1);
...
%         
        avedl1=mean((sumy*gradient(X1).^2).^0.5);
        Somm=zeros(1,kkk-1);
        parfor i = 1:(kkk-1)  %计算全序列的Som
            x0=X1(:,i);
            x1=X1(:,i+1);
            ...
            divD1 =divD(t,(x1+x0)/2,param);
            dF1 =dF(t,(x1+x0)/2,param);
            Veff1 =Veff(x0,x1,F1,dF1,G1,param,D,divD1,avedl1);
            
%             S_modified = S_modified + real(Som_i(x0,x1,F1, G1 ,param,D,E0,Veff1,avedl1)); %计算全序列的Som%%%%%%%%%%%%%%%%%%
            Somm(i)=Som_i(x0,x1,F1, G1 ,param,D,E0,Veff1,avedl1);
        end                        
        S_modified=real(sum(Somm));
...
Xt(i+1)=Xt(i) + dt(((x1-x0)'*(x1-x0))^0.5,3770+100,Veff(x0,x1,F1,dF1, G1,param,D,divD1,avedl));
end
