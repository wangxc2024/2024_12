 ...
 *
 * _coder_dFmatnum_mex_api.c
 *
 * Code generation for function '_coder_dFmatnum_mex_api'
 *
 */
...
static const mxArray *b_emlrt_marshallOut(const real_T u[90]);
...
static real_T emlrt_marshallIn(const emlrtStack *sp, const mxArray *t, const
  char_T *identifier);
static const mxArray *emlrt_marshallOut(const real_T u[8100]);
...
void dFmatnum_api(const mxArray * const prhs[3], int32_T nlhs, const mxArray
                  *plhs[1])
{
  real_T (*A)[8100];
  real_T t;
  ...
  A = (real_T (*)[8100])mxMalloc(sizeof(real_T [8100]));
                ...
                plhs[1])
{
  ...
  /* Marshall function outputs */
  plhs[0] = b_emlrt_marshallOut(*dx);
}

/* End of code generation (_coder_dFmatnum_mex_api.c) */
