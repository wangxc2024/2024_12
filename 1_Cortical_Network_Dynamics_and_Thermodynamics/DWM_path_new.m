...
% param.Iinput=0;
D=0.001;
Gn = @(x,param)  diffu(x,param,D,1)/D;
t=1;
...
Veff = @(x,x1,Fx,Fx1,Gn,param,D,divD,h,avedl) ...
       -( trace(dF) - (Fx+Fx1)'/2 * (D*Gn)^-1 * divD ) ... 
       - 0.5 * (Fx+Fx1)'/2 * (D*Gn)^-1 * (Fx+Fx1)/2 ;

Som_i = @(x,x1,Fx,Fx1,Gn,param,D,E,Veff,avedl) ...
        ( 2*(E-Veff) * ((x1-x)'*(x1-x)) ).^0.5 ...
        - (Fx+Fx1)'/2 * (D*Gn)^-1 * (x1-x) ...
        + 0.00001 * ( (x1-x)'*(x1-x) - avedl^2 ) ;
...
while kk < (1 + n_all) %进行500次退火优化并取平均
    ...
    parfor i = 1:(kkk-1)  %计算全序列的Som
            ...
            S_initial0 = S_initial0 + Som_i(X(:,i),X(:,i+1),F90(t,X(:,i),param), F90(t,X(:,i+1),param),Gn((X(:,i+1)+X(:,i))/2,param),param,D,h,avedl);%%%
    end
    while kkkk < n_anneal  %开始一次退火优化流程
        ...
        for i = 1:(kkk-1)
            h=dh(X(:,i),X(:,i+1),F90(t,X(:,i),param),F90(t,X(:,i+1),param),Gn((X(:,i+1)+X(:,i))/2,param),param,D);
            Xt(1,i+1)=Xt(1,i)+h;
        end
        avedl=mean((sumy*X.^2).^0.5);
        parfor i = 1:(kkk-1)  %计算全序列的Som
            ...
            h=Xt(1,i+1)-Xt(1,i);
            S_initial = S_initial + Som_i(X(:,i),X(:,i+1),F90(t,X(:,i),param),F90(t,X(:,i+1),param),Gn((X(:,i+1)+X(:,i))/2,param),param,D,h,avedl);%%%
        end
...
%             end
        ...
        for i = 1:(kkk-1)
            h=dh(X1(:,i),X1(:,i+1),F90(t,X1(:,i),param),F90(t,X1(:,i+1),param),Gn((X1(:,i+1)+X1(:,i))/2,param),param,D);
            Xt(1,i+1)=Xt(1,i)+h;
        end
        avedl=mean((sumy*X1.^2).^0.5);
        parfor i = 1:(kkk-1)  %计算全序列的Som
            h=Xt(1,i+1)-Xt(1,i);
            S_modified = S_modified + Som_i(X1(:,i),X1(:,i+1),F90(t,X1(:,i),param),F90(t,X1(:,i+1),param),Gn((X1(:,i+1)+X1(:,i))/2,param),param,D,h,avedl); %计算全序列的Som%%%%%%%%%%%%%%%%%%
        end                        
